...
typedef struct _TEB {
  ...
  ULONG
  Win32ClientInfo[31];        /* 044 used for user32 private data in Wine */
  PVOID           WOW32Reserved;              /* 0c0 */
  ULONG           CurrentLocale;              /* 0c4 */
  ULONG           FpSoftwareStatusRegister;   /* 0c8 */
  PVOID
  SystemReserved1[54];        /* 0cc used for kernel32 private data in Wine */
  PVOID           Spare1;                     /* 1a4 */
  LONG            ExceptionCode;              /* 1a8 */
  BYTE            SpareBytes1[40];            /* 1ac */
  PVOID
  SystemReserved2[10];        /* 1d4 used for ntdll private data in Wine */
...
NTSTATUS    NTAPI    NtAdjustPrivilegesToken(HANDLE, SIZE_T, PTOKEN_PRIVILEGES,
    SIZE_T, PTOKEN_PRIVILEGES, PDWORD);
NTSTATUS    NTAPI    NtAllocateVirtualMemory(HANDLE, PVOID *, SIZE_T, SIZE_T *,
    SIZE_T, SIZE_T);
NTSTATUS    NTAPI    NtCancelIoFile(HANDLE, PIO_STATUS_BLOCK);
NTSTATUS    NTAPI    NtClearEvent(HANDLE);
NTSTATUS    NTAPI    NtClose(HANDLE);
NTSTATUS    NTAPI    NtCreateEvent(PHANDLE, ACCESS_MASK,
                                   const OBJECT_ATTRIBUTES *, SIZE_T, SIZE_T);
NTSTATUS    NTAPI    NtCreateFile(PHANDLE, ACCESS_MASK, POBJECT_ATTRIBUTES,
                                  PIO_STATUS_BLOCK, PLARGE_INTEGER, SIZE_T, SIZE_T, SIZE_T, SIZE_T, PVOID,
                                  SIZE_T);
NTSTATUS    NTAPI    NtCreateKey(PHANDLE, ACCESS_MASK, POBJECT_ATTRIBUTES,
                                 SIZE_T, PUNICODE_STRING, SIZE_T, PULONG);
...
NTSTATUS    NTAPI    NtDeviceIoControlFile(HANDLE, HANDLE, PIO_APC_ROUTINE,
    PVOID, PIO_STATUS_BLOCK, SIZE_T, PVOID, SIZE_T, PVOID, SIZE_T);
...
NTSTATUS    NTAPI    NtMapViewOfSection(HANDLE, HANDLE, PVOID *, SIZE_T, SIZE_T,
                                        const LARGE_INTEGER *, SIZE_T *, SECTION_INHERIT, SIZE_T, SIZE_T);
NTSTATUS    NTAPI    NtOpenEvent(PHANDLE, ACCESS_MASK,
...
NTSTATUS    NTAPI    NtOpenSection(HANDLE *, ACCESS_MASK,
                                   const OBJECT_ATTRIBUTES *);
NTSTATUS    NTAPI    NtOpenSymbolicLinkObject(PHANDLE, ACCESS_MASK,
    POBJECT_ATTRIBUTES);
NTSTATUS    NTAPI    NtQueryDirectoryFile(HANDLE, HANDLE, PIO_APC_ROUTINE,
    PVOID, PIO_STATUS_BLOCK, PVOID, SIZE_T, FILE_INFORMATION_CLASS, SIZE_T,
    PUNICODE_STRING, SIZE_T);
NTSTATUS    NTAPI    NtQueryInformationFile(HANDLE, PIO_STATUS_BLOCK, PVOID,
    SIZE_T, FILE_INFORMATION_CLASS);
...
NTSTATUS    NTAPI    NtQueryKey(HANDLE, KEY_INFORMATION_CLASS, PVOID, SIZE_T,
                                PULONG);
NTSTATUS    NTAPI    NtEnumerateValueKey(HANDLE, ULONG,
    KEY_VALUE_INFORMATION_CLASS, PVOID, SIZE_T, PULONG);
NTSTATUS    NTAPI    NtQueryValueKey(HANDLE, PUNICODE_STRING,
                                     KEY_VALUE_INFORMATION_CLASS, PVOID, SIZE_T, PULONG);
NTSTATUS    NTAPI    NtQueryVolumeInformationFile(HANDLE, PIO_STATUS_BLOCK,
    PVOID, SIZE_T, FS_INFORMATION_CLASS);
NTSTATUS    NTAPI    NtReadFile(HANDLE, HANDLE, PIO_APC_ROUTINE, PVOID,
                                PIO_STATUS_BLOCK, PVOID, SIZE_T, PLARGE_INTEGER, PULONG);
...
NTSTATUS    NTAPI    NtSetSystemPowerState(POWER_ACTION SystemAction,
    SYSTEM_POWER_STATE MinSystemState, SIZE_T Flags);
NTSTATUS    NTAPI    NtSetValueKey(HANDLE, PUNICODE_STRING, SIZE_T, SIZE_T,
                                   PVOID, SIZE_T);
NTSTATUS    NTAPI    NtShutdownSystem(SHUTDOWN_ACTION);
NTSTATUS    NTAPI    NtTerminateProcess(HANDLE, SIZE_T);
NTSTATUS    NTAPI    NtWaitForSingleObject(HANDLE, SIZE_T,
    const LARGE_INTEGER *);
...
NTSTATUS    NTAPI    RtlAdjustPrivilege(SIZE_T Id, SIZE_T Enable,
                                        SIZE_T ForCurrentThread, SIZE_T *WasEnabled);
PVOID       NTAPI    RtlAllocateHeap(HANDLE, SIZE_T, SIZE_T);
NTSTATUS    NTAPI    RtlAnsiStringToUnicodeString(PUNICODE_STRING, PANSI_STRING,
    SIZE_T);
HANDLE      NTAPI    RtlCreateHeap(SIZE_T, PVOID, SIZE_T, SIZE_T, PVOID,
                                   PRTL_HEAP_DEFINITION);
BOOLEAN     NTAPI    RtlCreateUnicodeString(PUNICODE_STRING, LPCWSTR);
NTSTATUS    NTAPI    RtlCreateUserThread(HANDLE, PSECURITY_DESCRIPTOR, SIZE_T,
    SIZE_T, SIZE_T, SIZE_T, PTHREAD_START_ROUTINE, PVOID, PHANDLE, PCLIENT_ID);
HANDLE      NTAPI    RtlDestroyHeap(HANDLE);
BOOLEAN     NTAPI    RtlDosPathNameToNtPathName_U(PCWSTR, PUNICODE_STRING,
    PCWSTR *, CURDIR *);
/* VOID     NTAPI    RtlExitUserThread(NTSTATUS); - NEVER use this unreliable call! */
NTSTATUS    NTAPI    RtlExpandEnvironmentStrings_U(PWSTR,
    const UNICODE_STRING *, UNICODE_STRING *, ULONG *);
NTSTATUS    NTAPI    RtlFindMessage(PVOID BaseAddress, ULONG Type,
                                    ULONG Language, ULONG MessageId, MESSAGE_RESOURCE_ENTRY **MessageResourceEntry);
...
PRTL_USER_PROCESS_PARAMETERS NTAPI RtlNormalizeProcessParams(
  RTL_USER_PROCESS_PARAMETERS *);
ULONG       NTAPI    RtlNtStatusToDosError(NTSTATUS);
NTSTATUS    NTAPI    RtlQueryEnvironmentVariable_U(PWSTR, PUNICODE_STRING,
    PUNICODE_STRING);
NTSTATUS    NTAPI    RtlQueryRegistryValues(ULONG RelativeTo, PCWSTR Path,
    PRTL_QUERY_REGISTRY_TABLE QueryTable, PVOID Context, PVOID Environment);
NTSTATUS    NTAPI    RtlSetEnvironmentVariable(PWSTR, PUNICODE_STRING,
    PUNICODE_STRING);
NTSTATUS    NTAPI    RtlSystemTimeToLocalTime(const LARGE_INTEGER *SystemTime,
    PLARGE_INTEGER LocalTime);
VOID        NTAPI    RtlTimeToTimeFields(PLARGE_INTEGER Time,
...
NTSTATUS    NTAPI    RtlWriteRegistryValue(ULONG RelativeTo, PCWSTR Path,
    PCWSTR ValueName, ULONG ValueType, PVOID ValueData, ULONG ValueLength);

VOID        NTAPI    DbgBreakPoint(VOID);
NTSTATUS    NTAPI    LdrGetDllHandle(SIZE_T, SIZE_T, const UNICODE_STRING *,
                                     HMODULE *);
NTSTATUS    NTAPI    LdrGetProcedureAddress(PVOID, PANSI_STRING, SIZE_T,
    PVOID *);
