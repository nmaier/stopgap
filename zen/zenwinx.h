...
WINX_FILE *winx_fbopen(const wchar_t *filename, const char *mode,
                       int buffer_size);
...
typedef struct _winx_file_info {
  ...
  unsigned long
  flags;               /* combination of FILE_ATTRIBUTE_xxx flags defined in winnt.h */
  winx_file_disposition
  disp;        /* information about file fragments and their disposition */
  unsigned long
  user_defined_flags;  /* combination of flags defined by the caller */
  winx_file_internal_info
  internal;  /* internal information used by ftw_scan_disk support routines */
...
typedef void (*ftw_progress_callback)(winx_file_info *f,
...
int winx_ftw_dump_file(winx_file_info *f, ftw_terminator t,
                       void *user_defined_data);
...
int winx_patcomp(winx_patlist *patterns, wchar_t *string, wchar_t *delim,
                 int flags);
...
typedef int (*volume_region_callback)(winx_volume_region *reg,
                                      void *user_defined_data);
